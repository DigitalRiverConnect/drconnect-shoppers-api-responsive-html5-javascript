...
return c.promise}},load:function(a,b,c){return a&&a.uri?this.makeRequest(this.session.retrieve(a.uri,b),c):this.failRequest("The resource does not provide a URI",c)},getCallbacks:function(a){var b=this,c={};a||(a={});c.error=function(c){if(c.error&&c.error.errors&&c.error.errors.error)c.error=c.error.errors.error;a.error&&typeof a.error==="function"?(a.error({status:c.status,details:c}),a.callDefaultErrorHandler&&b.options.error&&typeof b.options.error==="function"&&b.options.error({status:c.status,
details:c})):b.options.error&&typeof b.options.error==="function"&&b.options.error({status:c.status,details:c})};c.success=function(c){a.success&&typeof a.success==="function"?a.success(c):a&&typeof a==="function"?a(c):b.options.success&&typeof b.options.success==="function"&&b.options.success(c)};return c}});
var buildUriFromOptions=function(a,b,c){return a+"?redirect_uri="+encodeURIComponent(b)+"&response_type=token&client_id="+c.client_id},getUriWithToken=function(a,b){return a+"&limited_token="+b},ns=namespace("dr.api.view");ns.AuthIFrameView=function(a,b,c){this.uri=buildUriFromOptions(a,b,c);this.id=dr.api.config.AUTH_FRAME_ID;this.parentElementId=c.elementId};
ns.AuthIFrameView.prototype.open=function(a,b){var c=document.getElementById(this.id);c||(c=this.create());var d=getUriWithToken(this.uri,a);c.onload=function(){this.src==d&&b()};c.src=d};ns.AuthIFrameView.prototype.close=function(){console.log("Closing Auth IFrame");var a=document.getElementById(this.id);a&&a.parentNode.removeChild(a)};
...
(new Date).getTime()/1E3+parseInt(a.expires_in);return a}).fail(function(){console.debug("[DR Api Library] Token failure. Application could not obtain an anonymous token using a refresh token.");b.refreshToken=null;var a={status:401,error:{}};a.error.errors={};a.error.errors.error={code:"refresh_token_invalid",description:"The Refresh Token is invalid"};throw a;})};ns.Session.prototype.forceRefreshToken=function(){return this.getRefreshToken()};
ns.Session.prototype.forceResetSession=function(){this.reset();return this.anonymousLogin()};ns.Session.prototype.reset=function(){this.refreshToken=this.token=null;this.authenticated=this.connected=!1;this.tokenExpirationTime=this.tokenStartTime=null};
...
connect:function(a){return this.makeRequest(this.session.anonymousLogin(),a)},forceRefreshToken:function(a){return this.makeRequest(this.session.forceRefreshToken(),a)},forceResetSession:function(a){return this.makeRequest(this.session.forceResetSession(),a)},login:function(a,b){return this.makeRequest(this.session.authenticate(b),a)},logout:function(a){return this.makeRequest(this.session.logout(),a)},disconnect:function(a){return this.makeRequest(this.session.disconnect(),a)},checkConnection:function(a){var b=
Q.defer();this.cart.get({fields:"id"},function(){a()});return b.promise},getSessionInfo:function(){return{clientId:this.session.apikey,connected:this.session.connected,authenticated:this.session.authenticated,token:this.session.token,refreshToken:this.session.refreshToken,tokenExpirationTime:this.session.tokenExpirationTime}}});
